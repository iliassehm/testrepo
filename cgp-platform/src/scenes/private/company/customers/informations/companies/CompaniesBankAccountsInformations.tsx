...
import { BankAccountsForm } from "./holdingForm/bankAccountsForm";
import { SelectBusiness } from "./holdingForm/selectBusiness";

const defaultCompanyFinancial: CompanyFinancialSchema = {
  banks: "",
  iban: "",
  bankAccountNumbers: "",
  bic: "",
...
export const CompaniesBankAccountsInformations = (props: Props) => {
  ...
  const businessUpdate = useMutation(
    async (bankAccounts: CompanyFinancialSchema) => {
      await gql.client.request(
        CompanyCustomersInformationsLogic.businessUpdate(),
        {
          ...
          bankAccounts: bankAccounts || defaultCompanyFinancial,
        }
      );
    },
    {
  ...
  const { handleSubmit, control } = useForm({
    resolver: zodResolver(companyFinancialSchema),
    values: props.bankAccounts,
  });
    ...
    <div className="flex flex-col pb-10">
      ...
      >
        ...
        <div className="flex w-full flex-col gap-y-10 gap-x-20">
          <BankAccountsForm control={control} handleBlur={handleBlur} />
        </div>
      </form>
    </div>
  );
